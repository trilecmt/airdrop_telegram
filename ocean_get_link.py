...
def claim_a_wallet(driver,wallet_name):
    print_message(f'Claim {wallet_name}')
    try:
        ...
        if element is  not None:
            time.sleep(3)
            print_message(f'click wallet claim')
            element.click()
            element=driver_helper.wait_element_appear(driver,value="//div[@class='claim cursor-pointer']",timeout=10)
            if element is not None:
                time.sleep(3)
                print_message(f'click claim {wallet_name}')
                element.click()
                time.sleep(15)
            print_message('click back')
            driver.back()
...
def exec():
    try:    
        ...
        try: 
            ...
            print_message("Check button Comfirm IP Address...")
            element=driver_helper.wait_element_appear(driver,value="//button[text()='Confirm']",timeout=5)
            if element is not None:
                print_message("Click button Comfirm IP Address...")  
                sleep(1,4)
                element.click()
                sleep(2,4)

            print_message("Check button Launch...")
            element=driver_helper.wait_element_appear(driver,value="//*[text()='Confirm']",timeout=5)
            if element is not None:
                print_message("Click Launch...")
                sleep(1,3)
                element.click()
                sleep(1,3)
            frame=driver_helper.wait_element_appear(driver,value="//iframe",timeout=10)    
            helper.utils.write_config(section='OCEAN',key='url',value=frame.get_property("src"))
